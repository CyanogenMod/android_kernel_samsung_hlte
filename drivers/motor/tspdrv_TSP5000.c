...
/* initialized in tspdrv_tsp5000_probe */
...
struct vibrator_platform_data vibrator_tsp5000_drvdata;

static int set_vibetonz(int timeout)
{
	int8_t strength;
	if (!timeout) {
		if (vibrator_tsp5000_drvdata.vib_model == HAPTIC_PWM) {
			strength = 0;
...
int vibetonz_ts2665_clk_on(struct device *dev)
{
...
int vibetonz_ts2665_clk_off(struct device *dev)
{
	struct clk *vibetonz_clk = NULL;
...
static int tspdrv_tsp5000_parse_dt(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	int rc;

	rc = of_property_read_u32(np, "samsung,vib_model", &vibrator_tsp5000_drvdata.vib_model);
	if (rc) {
		pr_err("%s:%d, vib_model not specified\n",
						__func__, __LINE__);
		return -EINVAL;
	}
	rc = of_property_read_u32(np, "samsung,pmic_haptic_pwr_en", &vibrator_tsp5000_drvdata.is_pmic_haptic_pwr_en);
	if (rc) {
...
static int suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (g_bIsPlaying)
	{
		...
		vibetonz_ts2665_clk_off(&pdev->dev);
...
static const struct of_device_id vib_motor_ts2665_match[] = {
	{.compatible = "ts2665_vibrator"},
	{}
};

static struct platform_driver tspdrv_driver = {
	.probe = tspdrv_tsp5000_probe,
	...
	.driver = {
		.name = MODULE_NAME,
		.owner = THIS_MODULE,
		.of_match_table = vib_motor_ts2665_match,
	},
};
