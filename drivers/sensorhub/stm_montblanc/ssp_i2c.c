	...
	u8 uSensorType, u8 *uSendBuf, u8 uLength)
{
	...
	case GO_SLEEP:
		...
		data->uLastResumeState = MSG2SSP_AP_STATUS_SLEEP;	
		break;
	case REMOVE_LIBRARY:
...
#endif
	msg->buffer[3] = 0; /*NU*/
	msg->buffer[4] = data->rot_direction;

	iRet = ssp_spi_async(data, msg);
#if defined(CONFIG_MACH_FLTESKT)
	pr_info("[SSP] Sensor Posision A : %u, G : %u, M: %u, P: %u, R: %u\n",
			msg->buffer[0], msg->buffer[1], msg->buffer[2],
			msg->buffer[3], msg->buffer[4]);
#else
	pr_info("[SSP] Sensor Posision A : %u, G : %u, M: %u, P: %u, R: %u\n",
			data->accel_position, data->accel_position, data->mag_position,
			0, data->rot_direction);
#endif
