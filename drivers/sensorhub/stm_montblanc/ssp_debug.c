...
void ssp_dump_task(struct work_struct *work) {
	...
	int iTimeTemp;
	mm_segment_t fs;
	int buf_len, packet_len, residue, iRet = 0, index = 0;
	...
	while (residue > 0) {
		...
		iRet = vfs_write(dump_file, (char __user *) buffer, packet_len,
				&dump_file->f_pos);
		if (iRet < 0) {
	...
	ssp_send_cmd(big->data, MSG2SSP_AP_MCU_DUMP_FINISH, iRet);
